...
struct PrintOptions
{
	...
	char date; 		//是否打印日期，0不打印，1打印，默认不打印
	char millisecond; //是否打印毫秒，0不打印，1打印，默认不打印
	char style;		//打印风格,0旧风格,1便于远程日志解析的风格,默认原始风格 
	char tologfile;	//是否写到日志文件。0写入，1只打印不写入。默认写入日志文件。
	char resv[19]; 	//保留
};
...
int INFRA_API getPrintLogLevel();
...
/// \printOptions 打印选项，参见PrintOptions结构体
/// \trturn 0 成功 -1 失败
int INFRA_API getPrintOptions(PrintOptions* printOptions);
...
///logfiter -logsavesetting bysize [logsize] [maxnumber] #logsize-log will save as a new file if log's size reaches to logsize;maxnumber-total log file;
///logfiter -logsavesetting byday [on/off] [maxnumber] #on-log will save as a new file if date changes;maxnumber-total log file;
///logfiter -logsavesetting none #log file save in the old way,never save as a new file;
