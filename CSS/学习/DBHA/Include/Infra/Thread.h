...
public:
	...
	/// \param policy 线程调度策略
	/// \param stackSize 线程指定的堆栈大小，单位: 字节。如果等于0或者小于平台要求必须的值，使用平台缺省值。
	///        请慎重设置线程栈大小，防止因线程栈溢出造成死机。
	///        历史原因，类Unix系统(Linux/MacOS等)默认不生效，Windows可用。Unix系统需调用 enableStackSize() 接口使能该参数。
	...
	/// 创建线程，如果希望构造函数的 stackSize 生效，创建前先调用 enableStackSize() 接口
	bool createThread();
	...
	/// 使构造函数传入的 stackSize 参数生效。必须在 createThread 接口之前调用。
	void enableStackSize();
