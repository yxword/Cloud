...
public:
	...
	//设置文件的ＩＯ缓冲模式，在open接口之后，其他任何接口之前调用有效
	/// \param [in] buf 缓冲区指针
	/// \param [in]mode 缓冲模式
	/// \param [in]mode 缓冲区大小
	/// \retval true  设置成功
	/// \retval false  设置失败
	virtual bool setvbuf(char *buf, IOMode mode, size_t size);

	virtual bool resv1(){return false;}
	virtual bool resv2(){return false;}
	virtual bool resv3(){return false;}
	virtual bool resv4(){return false;}
	...
	/// 递归创建目录，上一级不存在路径一并创建
	/// \param dirPath 目录名	
	static bool makeDirectoryRecursively(const char* dirPath);
	...
	/// 删除目录,目录里的内容会一起删除
	/// \param dirPath 目录名全路径
	/// \param rmOriginalDir 是否删除原始目录，默认删除；false则只删除该目录下的内容保留目录本身
	static bool removeDirectoryRecursively(const char* dirPath, bool rmOriginalDir = true);
...
public:
	...
	enum Attribute {
		...
		// posix 兼容的文件属性掩码
		posixFIFO		= 0x10000,
		posixCharacter	= 0x20000,	///< Character special (indicates a device if set)
		posixDirectory	= 0x40000,
		posixRegular	= 0x80000,
