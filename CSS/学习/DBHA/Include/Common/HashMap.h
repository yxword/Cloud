...
public:
	...
	explicit CHashMap(void* pridata,HashMapFuncs& funcs, uint64_t size);
	~CHashMap();
...
public:
	...
	Internel()
	{
		...
		m_hash_funcs.hash = NULL;
		m_hash_funcs.keydup = NULL;
		m_hash_funcs.valdup = NULL;
		m_hash_funcs.keycmp = NULL;
		m_hash_funcs.keyrelease = NULL;
		m_hash_funcs.valrelease = NULL;
	}
...
template<typename K,typename V>
CHashMap<K,V>::CHashMap( void *pridata ,HashMapFuncs& funcs, uint64_t size)
{
	m_internel = new Internel;
	assert(NULL != m_internel );
	...
	dictExpand(m_internel->m_dict, size);
