...
public:
   ...
   //初始化locked=true则外部通过lock(k)加锁保证线程安全
    ...
    //两个linkedhashmap交换部分子hash,调用该函数要求两个CLinkedHashMap的hash函数应该一致.
    //locked=true线程安全
    void swap(CLinkedHashMap<K,V>& rhs, uint32_t id);
...
template<typename K,typename V>
void CLinkedHashMap<K,V>::swap(CLinkedHashMap<K,V>& rhs, uint32_t id)
{
    if( m_need_locked ){
        Infra::CGuard lock(m_slot_mutex[id]);
        m_linked_map[id]->swap(*rhs.m_linked_map[id]);
    }else{
        m_linked_map[id]->swap(*rhs.m_linked_map[id]);
    }
}
