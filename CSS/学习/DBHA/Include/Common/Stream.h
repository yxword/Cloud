...
public:
	CStream();
	/// 绑定在网络框架私有线程池上，idx为私有线程池ID
	/// 此ServiceBase子类接口被私有线程池调度
	CStream(int32_t idx);
	~CStream();	
	enum
	{
	...
	bool setNoDelay( bool noDelay = true );
	...
	//重置远端和本地addr，等待下次重连时生效；如果要立即生效，外部需要调用reset接口
	void resetAddr( NetFramework::CSockAddr *remote, NetFramework::CSockAddr *local = NULL );
private:
	class CStreamCore;
