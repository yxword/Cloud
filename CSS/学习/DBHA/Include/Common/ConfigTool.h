...
public:
    ...
    // 从ConfigTool中重新加载一个项配置
    // domain 自定义的配置域。
    // keys 指定重新加载的配置项。
    // 返回值： 成功时返回false，失败时返回true。
    bool reloadConfig( const std::string& domain, const std::vector<std::string>& keys );
    ...
    bool  getUInt(const std::string& domain, const std::string& key, uint32_t& val);
    bool  setUInt(const std::string& domain, const std::string& key, const uint32_t val);
