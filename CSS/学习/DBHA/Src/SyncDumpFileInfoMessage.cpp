...
DBHALOG_CLASSNAME( CSyncDumpFileInfoMessage );

using namespace std;
using namespace Infra;
using namespace EFS;

class CSyncDumpFileInfoMessage::Internal
{
public:
	Internal( EFS::ProtoHeader* header )
	{
		...
		m_header_ptr->cmd = HAFK_SYNC_DUMP_FILE_INFO_MESSAGE;
		m_header_ptr->seq = 1;
	...
	{
		DBHALOG_INFO( "CSynDumpFileInfoMessage: type:%u, cmd:%u seq:%u bodyLength:%u\n",
...
CSyncDumpFileInfoMessage::CSyncDumpFileInfoMessage()
{
	m_internal = new Internal( &m_header );
	assert( NULL != m_internal );
}

CSyncDumpFileInfoMessage::CSyncDumpFileInfoMessage( uint32_t seq )
{
	m_internal = new Internal( &m_header );
	assert( NULL != m_internal );
...
bool CSyncDumpFileInfoMessage::append( const char* buf, uint32_t len )
{
	return m_internal->append( buf, len );
}
//获取序列化的数据
const char* CSyncDumpFileInfoMessage::getData()
{
	return m_internal->getData();
}
//获取序列化的数据的长度
uint32_t CSyncDumpFileInfoMessage::getDataLen()
{
	return m_internal->getDataLen();
}
//打印包用于debug
void CSyncDumpFileInfoMessage::printPacket() const
{
	m_internal->printPacket();
}

class CResSyncDumpFileInfoMessage::Internal
{
public:
	Internal( EFS::ProtoHeader* header )
	{
	...
	void printPacket() const
	{
		DBHALOG_DEBUG( "CResSyncGetVerMessage: type:%u, cmd:%u seq:%u bodyLength:%u\n",
			m_header_ptr->type,m_header_ptr->cmd,m_header_ptr->seq,m_header_ptr->bodyLen );
	...
	//获取Json协议.
	void getDumpFileInfo(std::string& fileName, uint64_t& fileLen )
	{
...
private:
	...
	bool serialize()
	{
		...
		m_header_ptr->bodyLen += INT64_SIZE;
		m_header_ptr->bodyLen += sizeOfString(m_file_name);
		int64_t total_len = sizeof( EFS::ProtoHeader ) + m_header_ptr->bodyLen;
		m_sn.setSerialize( total_len );
		if( EFS_SUCCESS != m_sn.writeBytes( m_header_ptr, sizeof( EFS::ProtoHeader ) ) ) return false;
...
CResSyncDumpFileInfoMessage::CResSyncDumpFileInfoMessage()
{
	m_internal = new Internal( &m_header );
	assert( NULL != m_internal );
}
CResSyncDumpFileInfoMessage::CResSyncDumpFileInfoMessage( uint32_t seq )
{
	m_internal = new Internal( &m_header );
	assert( NULL != m_internal );
	m_internal->setSequence( seq );
...
//尾部插入方式增加原始数据( 序列化后的 ).
bool CResSyncDumpFileInfoMessage::append( const char* buf, uint32_t len )
{
	return m_internal->append( buf, len );
}
//获取序列化的数据.
const char* CResSyncDumpFileInfoMessage::getData()
{
	return m_internal->getData();
}
//获取序列化的数据的长度.
uint32_t CResSyncDumpFileInfoMessage::getDataLen()
{
	return m_internal->getDataLen();
}
//打印包用于debug.
void CResSyncDumpFileInfoMessage::printPacket() const
{
	m_internal->printPacket();
}
...
//协议内容.
//包有错误,则设置错误类型进去,否则默认包是正确的.
void CResSyncDumpFileInfoMessage::setErrorType( int32_t errorType )
{
	m_internal->setErrorType( errorType );
}
//获取包是否有错,0表示正确,-1标识错误.
int8_t CResSyncDumpFileInfoMessage::isError() const
{
	return m_internal->isError();
}
//如果包出错,则获取到的错误类型标识是什么错误.
int32_t CResSyncDumpFileInfoMessage::getErrorType() const
{
	return m_internal->getErrorType();
}

//设置版本号.
void CResSyncDumpFileInfoMessage::setDumpFileInfo(string fileName, uint64_t len)
{
	m_internal->setDumpFileInfo(fileName, len);
}

//获取Json协议.
void CResSyncDumpFileInfoMessage::getDumpFileInfo(string& fileName, uint64_t& len)
{
	m_internal->getDumpFileInfo(fileName, len);
}

EFS::TPacketBasePtr newCSyncDumpFileInfoMessage()
{
	return EFS::TPacketBasePtr( new CSyncDumpFileInfoMessage() );
}

EFS::TPacketBasePtr newCResSyncDumpFileInfoMessage()
{
	return EFS::TPacketBasePtr( new CResSyncDumpFileInfoMessage() );
}
