...
int CTcpCli::StartSession( CSockAddr & addr )
{
...
int CTcpCli::handle_output( int handle )
{
...
int CTcpCli::handle_output_timeout( int handle )
{
	printf( "超时！\n" );
...
int CTcpCli::handle_exception( int handle )
{
	if( handle == m_stream.GetHandle() ){
		printf( "连接发生异常!\n" );
		RemoveSock( m_stream );
...
int CTcpCli::handle_close(CNetHandler *myself)
{
	delete myself;
	return 0;
}
...
int StartTcpCli()
{
	CTcpCli *tcpcli = new CTcpCli;		
	CSockAddrIPv4 addr( "10.6.5.215", 9000 );	
	if( tcpcli->StartSession( addr ) < 0 )
	{	
		printf("tcp client start session failed\n");
		return -1;
	}
