...
#ifdef NDEBUG
#define ASSERT(expression,flag) do{if(!(expression)){Dahua::Infra::fatalf("Assert fail:%s, file %s, function %s, line %d\n",\
	#expression,__FILE__,__FUNCTION__,__LINE__);if(flag) abort();}}while(0)
	
#define ASSERT_RET(expression, ret) do{if(!(expression)){Dahua::Infra::fatalf("Assert fail:%s, file %s, function %s, line %d\n",\
