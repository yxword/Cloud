...
template<class T> inline void swap(Memory::TScopedPtr<T>& a, Memory::TScopedPtr<T>& b) // never throws
{
    a.swap(b);
}
...
template<class T> inline T * get_pointer(Memory::TScopedPtr<T> const & p)
{
    return p.get();
}
